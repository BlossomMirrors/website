...
<button
	...
	onmouseenter={() => { internalHovered = true; onmouseenter?.(); }}
	onmouseleave={() => { internalHovered = false; onmouseleave?.(); }}
	{onclick}
	{onpointerdown}
>
	<svg
	...
	>
		{#if hovered}
			{#if type === 'close'}
				<rect
					x="0.5" y="0.5" width="15" height="15" rx="2.5"
					fill="#FF665F"
		...
		{#if type === 'minimize'}
			<path d="M4 8H12" stroke={stroke} stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" />
		{:else if type === 'maximize'}
			...
			/>
		{:else if type === 'close'}
			<path d="M11.5 4.5L4.5 11.5" stroke={stroke} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
			<path d="M4.5 4.5L11.5 11.5" stroke={stroke} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
		{/if}
	</svg>
</button>

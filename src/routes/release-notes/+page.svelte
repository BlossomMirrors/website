...
<style>
	...
	@keyframes paperFadeIn {
		from {
			opacity: 0;
			transform: rotateX(15deg) rotateZ(-5deg) translateY(30px);
		}
		to {
			opacity: 1;
			transform: rotateX(5deg) rotateZ(-2deg) translateY(0);
		}
	}
	...
	.divider {
		height: 2px;
		background: linear-gradient(90deg, transparent, #6666ff, transparent);
		margin: 2rem 0;
	}

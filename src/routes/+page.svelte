	...
	const products = [
		{
			Icon: MonitorIcon,
			name: 'BlossomOS',
			tagline: m.home_product_os_tagline(),
			href: '/os',
		...
		},
		{
			...
			name: 'Arc Software',
			tagline: m.home_product_arc_tagline(),
			href: '/arc',
			cta: m.home_product_learn_more()
		},
		{
			Icon: CloudIcon,
			name: 'Blossom Cloud',
			tagline: m.home_product_cloud_tagline(),
			href: '/cloud',
			cta: m.home_product_learn_more()
		},
		{
			Icon: Building2Icon,
			name: 'Blossom Hub',
			tagline: m.home_product_hub_tagline(),
			href: '/hub',
			cta: m.coming_soon(),
			soon: true
		}
...
<!-- Hero -->
<div class="flex flex-col items-center py-20 text-center md:py-32" use:reveal>
	<p class="mb-4 text-xs font-semibold tracking-widest text-primary uppercase">
		{m.home_tagline()}
	</p>
	<h1 class="font-serif text-5xl leading-tight md:text-7xl lg:text-8xl">
		...
		{#each products as p, i (p.name)}
			...
			>
				...
				<div class="flex-1">
					...
					<p class="mt-1 text-sm leading-relaxed text-muted-foreground">{p.tagline}</p>
				</div>
...
>
	...
	<div class="relative max-w-2xl">
		<h2 class="font-serif text-4xl leading-tight md:text-5xl">
			{m.home_mission_h2_1()}<br />{m.home_mission_h2_2()}
		</h2>
		...
		<div class="mt-8 flex flex-wrap gap-3">
			<a href="/about">
				<Button variant="primary">{m.home_mission_about()}</Button>
			</a>
